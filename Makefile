...
build-masp-indexer:
	docker build --no-cache -t ${registry_url}/masp-indexer-webserver:latest --build-arg GIT_SHA=${masp_sha}
	docker build --no-cache -t ${registry_url}/masp-indexer-chain:latest --build-arg GIT_SHA=${masp_sha}
	docker build --no-cache -t ${registry_url}/masp-indexer-block-filter:latest --build-arg GIT_SHA=${masp_sha}