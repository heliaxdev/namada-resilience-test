...
impl StepContext for Shielding {
    ...
    fn assert(&self, code: &Code) {
        ...
        if is_fatal {
            assert_step!("Fatal Shielding", details)
        } else if is_failed {
            assert_step!("Failed Shielding", details)
        } else if is_skipped {
            assert_step!("Skipped Shielding", details)
        } else if is_successful {
            assert_step!("Done Shielding", details)
        } else {
            assert_step!("Unknown Code Shielding ", details)
        }
    }
}
